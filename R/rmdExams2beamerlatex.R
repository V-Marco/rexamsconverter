...
RmdExams2beamerlatex <- function(path2RmdFilesFolder) {
  ...
  write('\\title{Midterm 2018} \n \\subtitle{Probability and statistics} \n \\author{See: \\url{https://github.com/bdemeshev/probability_hse_exams}} \n \\date{Last update: \\today}', path, append = TRUE)
  ...
  while (counter <= n_files - 1) {
    ...
    question_right <- paste("\n \\begin{frame} \\label{",
                            counter, "-Yes} \n", '\\begin{block}{', counter, '} \n',  task, '\n \\end{block} \n', '\\begin{enumerate} \n', answer_string, '\\end{enumerate} \n', '\n \\textbf{Yes!} \n \\hyperlink{', (counter + 1), '}{\\beamerbutton{Next question}}', "\\end{frame} \n", sep = "")
